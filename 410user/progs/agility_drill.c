...
void * chase(void * arg) {
  ...
  snprintf(buf, sizeof (buf), "Starting chase thread with creation number %d", my_creation_number);
  REPORT_MISC(buf);
  ...
  snprintf(buf, sizeof (buf), "Successful finish for thread with creation number %d", my_creation_number);
  REPORT_MISC(buf);
