...
#define lprintf_kern(fmt,arg...) printf(fmt,##arg)
...
void output_list(int_list_head_t *myHead)
{
  ...
  for(celem=Q_GET_TAIL(myHead);celem != NULL; celem = Q_GET_PREV(myHead,celem,myLink))
  {
    lprintf_kern("%d <- ", celem->data);
  }
  lprintf_kern("End\n");
...
int main(int argc, char **argv)
{
  ...
  {
    curList = Q_GET_NEXT(head,curList,myLink);
  }
